...
st <- "Global demand for coal is at an all-time high, but looking at the
changes in coal production per capita show that the story of coal production
is very different across the globe. In the last 20 years, Venezuela has 
drastically decreased coal production."
highlight_country <- "Venezuela"

# plot
ggplot(filter(plot_data, country != "Indonesia")) +
  geom_line(
    mapping = aes(
      x = year, y = coal_prod_per_capita, group = country
    ),
