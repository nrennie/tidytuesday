...
p <- ggplot(p_data, aes(x = trait,
  ...
  geom_image(data = slice(p_data, n = 1),
             aes(x = 1,
                 y = -40,
                 image = paste0("2022/2022-08-16/images/", slice(p_data, n = 1) %>% pull(name), "-modified.png")),
             size = 0.18) +
  ...
  theme(plot.background = element_rect(fill = "black", colour = "black"),
        ...
        plot.title = element_text(colour = "white", hjust = 0.5, family = "metal", size = 24),
        plot.caption = element_text(colour="white", size=10, hjust = 0.5, family = "montserrat"),
  )

# add image
img <- magick::image_read("2022/2022-08-16/logo.png")

ggdraw() + 
  draw_plot(p) + 
  theme(plot.background = element_rect(fill = "black", colour = "black")) + 
  draw_image(img, width = 0.3, x = 0.35, y = 0.4) 
